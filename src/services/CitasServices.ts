...
export async function updateAppointment(update: Cita) {
  const response = await fetch(`${API_URL}/citas/${update.cita_id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      ...
      tipo_procedimiento_id: update.tipo_procedimiento_id,
...
export async function searchAppointment(search: string) {
  const response = await fetch(`${API_URL}/citas/search`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
